	...
	public void updateView(final boolean _notify) {
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (notify) {
					Intent i = new Intent();
					i.setClassName("de.dotknowledge.xymon", "de.dotknowledge.xymon.Monitor");
					i.setFlags(Intent.FLAG_ACTIVITY_BROUGHT_TO_FRONT);
					
					PendingIntent pi=PendingIntent.getActivity(mainView.getContext(), 0, i, 0);
					
					Notification note = new Notification(currentStatus.getStatusIcon(), getString(R.string.app_name) + ": " + 
